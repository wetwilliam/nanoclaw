...
// Mock group-folder to return a predictable path
vi.mock('../group-folder.js', () => ({
  resolveGroupFolderPath: vi.fn().mockReturnValue('/tmp/nanoclaw-test'),
}));
...
vi.mock('grammy', () => ({
  Bot: class MockBot {
    ...
    api = {
      ...
      getFile: vi.fn().mockResolvedValue({ file_path: 'test/file.bin' }),
    };
...
describe('TelegramChannel', () => {
  ...
  describe('non-text messages', () => {
    it('stores photo with download path', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      const ctx = createMediaCtx({
        extra: { photo: [{ file_id: 'photo123' }] },
      });
      ...
      expect(opts.onMessage).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          content: expect.stringMatching(
            /^\[Photo: \/workspace\/group\/images\/photo_\d+\.\w+\]$/,
          ),
        }),
      );
    });
    ...
    it('stores photo with caption', async () => {
      ...
      expect(opts.onMessage).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          content: expect.stringMatching(
            /^\[Photo: \/workspace\/group\/images\/photo_\d+\.\w+\] Look at this$/,
          ),
        }),
      );
    });
    ...
    it('stores document with download path', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      const ctx = createMediaCtx({
        extra: { document: { file_name: 'report.pdf', file_id: 'doc123' } },
      });
      ...
      expect(opts.onMessage).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          content: expect.stringMatching(
            /^\[Document: \/workspace\/group\/files\/\d+_report\.pdf\]$/,
          ),
        }),
      );
    });
    ...
    it('stores document with fallback name when filename missing', async () => {
      ...
      expect(opts.onMessage).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          content: expect.stringMatching(
            /^\[Document: \/workspace\/group\/files\/\d+_file\]$/,
          ),
        }),
      );
    });

    it('falls back to error message when document download fails', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
    ...
    it('includes caption in document message', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
      ...
      expect(opts.onMessage).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          content: expect.stringMatching(
            /^\[Document: \/workspace\/group\/files\/\d+_report\.pdf\] My document$/,
          ),
        }),
      );
    });

    it('ignores document from unregistered chat', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
