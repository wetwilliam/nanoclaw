...
export class TelegramChannel implements Channel {
  ...
  async connect(): Promise<void> {
    ...
    this.bot.on('message:photo', async (ctx) => {
      ...
      try {
        ...
        const filename = await this.downloadPhoto(photo.file_id, group.folder);
        content = `[Photo: /workspace/group/images/${filename}]${caption}`;
        logger.info({ chatJid, filename }, 'Photo downloaded');
      } catch (err) {
    ...
    this.bot.on('message:document', async (ctx) => {
      ...
      try {
        ...
        content = `[Document: ${originalName} (download failed)]${caption}`;
      }
  ...
  private async downloadPhoto(fileId: string, folder: string): Promise<string> {
    ...
    const imagesDir = path.join(resolveGroupFolderPath(folder), 'images');
  ...
  private async downloadFile(
    ...
    const filesDir = path.join(resolveGroupFolderPath(folder), 'files');
